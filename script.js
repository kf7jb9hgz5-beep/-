...
function updateCanvas() {
    ...
    if (textWrapper) {
        ...
        // 기본 폰트/행간/자간 먼저 적용 (fitTextToCanvas에서 조정됨)
...
function fitTextToCanvas() {
    ...
    const ratio = baseLineHeight / baseFontSize; // 행간 비율 유지

    const areaWidth = area.getBoundingClientRect().width || parseFloat(area.style.width) || 420;
    const targetHeight = (areaWidth * h) / w;

    // 1. 일단 높이 auto로 풀어서 자연 높이 측정
    ...
    // 2. 이진 탐색으로 딱 맞는 폰트 크기 찾기
    let lo = 4;
    let hi = baseFontSize;

    for (let i = 0; i < 20; i++) {
        const mid = (lo + hi) / 2;
        textWrapper.style.fontSize = `${mid}px`;
        textWrapper.style.lineHeight = `${mid * ratio}px`;

        const curH = area.scrollHeight;
        if (curH <= targetHeight + 2) {
            lo = mid;
        } else {
            hi = mid;
        }
    ...
    // 3. 최종 적용
    textWrapper.style.fontSize = `${lo}px`;
    textWrapper.style.lineHeight = `${lo * ratio}px`;
    area.style.height = `${Math.round(targetHeight)}px`;
    area.style.overflow = "hidden";
}
